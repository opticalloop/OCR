...
static void printResult(double expected[], Neuron neuron[])
{
    ...
    for (unsigned int k = 0; k < 9; k++){
        printf("Output : %f, expected : %u\n", neuron[k].value, (unsigned int) expected[k]);
    }
}
...
static void createAllData(char* directory, char* intputPaths[], double input[][], double expected[][])
{
...
int main(int argc, char **argv)
{
    ...
    for (unsigned int i = 0; i <= epoch; i++){
        ...
        for (unsigned int j = 0; j < 3; j++){ 
            ...
            if (i % (epoch / 10) == 0){  
                printResult(expected[j], network->layers[nbHiddenLayers + 1].neurons);
            }
        }
    }
