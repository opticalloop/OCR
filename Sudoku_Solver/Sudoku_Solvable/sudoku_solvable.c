    ...
    for (unsigned int x = 0; x < dim; x++){
        for (unsigned int y = 0; y < dim; y++){
            ...
            if (isRowSolvable(grid, x) == false){
                printf("IsSolvable returned false with row check : x = %d and y = %d\n", x, y);
                return false;
            }

            if (isColumnSolvable(grid, y) == false){
                printf("IsSolvable returned false with column check : x = %d and y = %d\n", x, y);
                return false;
            }

            // Check square only when enter it
            if (y % 3 == 0 && x % 3 == 0 && isSquareSolvable(grid, x, y) == false){
                printf("IsSolvable returned false with square check : x = %d and y = %d\n", x, y);
                return false;
            }
        }
    }
