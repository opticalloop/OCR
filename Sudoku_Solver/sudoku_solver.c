...
unsigned int canBePlaced(unsigned int grid[dim][dim], unsigned int x, unsigned int y, unsigned int val)
{
    // Check row
...
unsigned int solveSuduko(unsigned int grid[dim][dim], unsigned int x, unsigned int y)
{
    // End of recursion
    if (x >= 8 && y > 8){
        return true;
